...
if __name__ == '__main__':
    ...
    fig, axs = plt.subplots(len(TRAINABLE_VARIABLES), 2, figsize=(10, 4*len(TRAINABLE_VARIABLES)))

    for i, (varname, filename) in enumerate(TRAINABLE_VARIABLES):

        ax1 = axs[i, 0] # Training loss plot
        ax2 = axs[i, 1] # Validation loss plot

        ax1.set_title(f'{SYMBOLS[varname]} - Training Loss')
        ax2.set_title(f'{SYMBOLS[varname]} - Validation Loss')
        ...
        for region_name, idx in TRAINABLE_REGIONS:
            ...
            losses, val_losses = train_for_region(varname, filename, idx, region_name)

            ax1.plot(losses, label=region_name)
            ax2.plot(val_losses, label=region_name)
